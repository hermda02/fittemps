program template_fitting
  ...
  if  (iargc() < 4) then
     ...
     if (trim(arg1) == 'help') then
        ...
        write(*,*) '   (version)/ -- (directory where all version information is held)'
        write(*,*) ''
        write(*,*) 'In sub-directories:'
        write(*,*) '   ./masks/*              -- (can be found at the github address)'
        write(*,*) '   ./maps/                -- (all maps listed in bands.txt)'
        write(*,*) '   ./(version)/templates/ -- (templates for each component for each band)'
        write(*,*) '   ./(version)/offset.dat -- (contains band offsets, in order)'
        write(*,*) '   ./(version)/gains.dat  -- (contains band gains, in order)'
        write(*,*) ''
        write(*,*) 'The output weights and maps will be written to:'
        write(*,*) '   ./(version)/(foreground)/'
        write(*,*) ''
        write(*,*) ''
